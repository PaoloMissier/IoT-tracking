...
\section{Removing the need  for trust}  \label{sec:no-trust}
...
In contrast, traffic summaries generated by marketplace participants reflect the \textit{local} view of each participant in the data exchanges, and are therefore necessarily partial and incomplete, as each participant, unlike the broker, has no visibility of the end-to-end data flows. 
We must therefore modify our definition of cubes slightly, and we denote these as \textit{unilateral} traffic cubes.

Specifically, assuming that a producer does not know which VAS subscribe to its stream, a publisher will produce a \textit{publisher's cube} $\mathit{cube^p}$ of the form
\[
...
where $N^s_{ik}(W)$ is the count of messages with topic $t_k$ sent by $p_i$ during $W$.
 Note that a publisher's cube is a slice of a complete traffic cube, for a specific producer and with the consumer dimension removed from it.

Subscribers, in contrast, know the source of the messages they receive. We are therefore going to assume that a subscriber will produce summary reports that include the publisher dimension, but which only contain the tuples that pertain to a single $c_j$. We refer to this as a \textit{subscriber's cube}, $ \mathit{cube^s} $.

Let 
\[  \mathit{sub}(t_k) \subseteq C \]
denote the set of subscribers to $t_k$.
We define:
\[
\mathit{cube^s}(W, c_j)  =  \{ \langle p_i, t_k, N_{ijk}(W) | c_j \in \mathit{sub}(t_k)\}_{p_i \in P, t_k \in T}
\]


\subsection{Validation of unilateral cubes}

A number of scenarios may unfold in this new setting where multiple unilateral reports are generated and collected for settlement.
Firstly, let us assume that each $p_i$ and $c_j$ each dutifully produce  $ \mathit{cube}^p(W, p_i) $ and $ \mathit{cube^s}(W, c_j)  $, respectively. 
As each of these cubes provides a partial view of the same complete cube that would have been generated centrally by a broker, we expect that the values found in these cubes must somehow be consistent with each other.
Specifically, note that each message from $p_i$ with topic $t_k$ will be received by each $c_j \in \mathit{sub}(t_k)$.
Thus, if $p_i$ sends 
$ \mathit{cube}^p(W, p_i)[t_k]  = N^s_{ik}(W)$ messages during $W$, then every $c_j \in \mathit{sub}(t_k)$ will have received the same number of messages from $p_i$. 
In other words, for any $p_i \in P, c_j \in C, t_k \in T$, the following constraint must hold:
\begin{equation} \label{eq:cubes-consistency}
\mathit{cube^s}(W, c_j)[p_i, t_k] = 
\begin{cases}
\mathit{cube}^p(W, p_i)[t_k] \quad \text{ if } c_j \in \mathit{sub}(t_k) \\
0 \text{ otherwise }
\end{cases}
\end{equation}

We say that the set of unilateral cubes $ \{ \mathit{cube}^p(W, p_i) \}_{p_i \in P}\cup \{\mathit{cube^s}(W, c_j) \}_{c_j \in C}$ are \textit{consistent} if and only if the cubes satisfy constraint (\ref{eq:cubes-consistency}).

A second scenario unfolds when only some of the $p_i$ and $c_j$, but not all, produce their cubes at the end of $W$.
In this case, there will be some  $ \mathit{cube}^p(W, p_i) $ with values that are not found in any corresponding $ \mathit{cube^s}(W, c_j) $, and vice versa.

We may use (\ref{eq:cubes-consistency}) to propagate the available cube values for all combinations of  $p_i$ and $c_j$.

\note{TBC}

\subsection{Settlement with unilateral cubes}

\authornote{when the cubes are consistent as defined above, we can settle just like in the centralised case.

same when cube counts propagation (TBD above) can be performed successfully.

trouble is when inconsistencies are spotted
}
...
\note{FIXME -- still a dump of Michele's text}
...
\subsection{Settlement using unilateral traffic cubes}
