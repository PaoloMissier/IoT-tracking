...
%
Note that the Gas price in our private test network is set as the same as the public Ethereum network.
%
%Our code will be publicly released on github.
%
...
\end{algorithm}
%
When a dispute resolution is invoked, payments are retained from being performed due to impossibility to clearly identify the correct unilateral cube. However a reputation mechanism can be implemented in order to penalize both parties involved in a given settlement transaction and to ultimately promote honest behavior. 
%
In its simplest form we expect that a reputation mechanism will require updating a special reputation account without any off-chain interaction~\cite{schaub2016trustless,carboni2015feedback}. Thus, the additional cost of supporting such a mechanism would be the same of performing a basic transaction (e.g., 21000 Gas). 
%
We are confident that this will not significantly affect the overall contract execution costs. 
%
%As it is not expected that reputation computation will require off-chain interaction~\cite{schaub2016trustless,carboni2015feedback}, we are confident that while not considering its implementation in this phase this will not significantly affect the overall contract execution costs. 
%
%More specifically, a simple reputation based mechanism can be implemented by issuing to each user a special account that stores the reputation value. The dispute resolution function can interact with the reputation account of each party involved in the settlment by performing basic transactions that change their state according to the settlement outcome. Thus, the additional cost of supporting such a reputation based mechanism would be 42000 Gas.
%
