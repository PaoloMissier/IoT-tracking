...
int mqtt3_db_message_write(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	while(tail){
		...
		if(tail->state != mosq_ms_queued){
			...
			/*** START INTERCEPT ***/
			_mosquitto_log_printf(NULL, MOSQ_LOG_DEBUG, "START INTERCEPT");
			time_t now = time(0);
			...
			// Connect session to keyspace
			CassFuture* connect_future = cass_session_connect_keyspace(session, cluster, "brokertracker");
